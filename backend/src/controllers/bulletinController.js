...
const { adressesFamille, envoyerALaFamille } = require('../services/familleService');
...
async function obtenirBulletin(req, res) {
  ...
  // Un Étudiant (ou son parent, connecté sur le même compte) ne consulte
  // que son propre bulletin.
...
async function envoyerBulletinParEmail(req, res) {
  ...
  if (adressesFamille(eleve).length === 0) {
    return res.status(400).json({ erreur: 'aucune adresse e-mail (élève ou parent) pour cet élève' });
  }
  ...
  const destinataires = await envoyerALaFamille(
    eleve,
  ...
  return res.json({ message: 'bulletin envoyé par e-mail', destinataires });
}
